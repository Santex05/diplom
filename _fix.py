  ...
  return (
    <div className="fixed bottom-4 right-4 z-[9000] w-[min(100%,24rem)] overflow-hidden rounded-2xl border border-white/10 bg-bg-card shadow-2xl">
      <div className="flex gap-3 p-3">
        ...
        <motion-safe-actions className="flex shrink-0 flex-col gap-1">
        ...
        <ul className="max-h-56 overflow-y-auto border-t border-white/[0.06] catalog-scroll">
          {items.map((item) => (
            <li key={item.id} className="flex items-center gap-2 border-b border-white/[0.04] px-3 py-2 last:border-0">
              ...
              </motionmotionmotionmotionmotionp>
    ...
    </motionmotionmotionmotionmotionmotiondiv>
  )
}
'''
content = re.sub(r'</motion[^>]*>', '</div>', content)
content = re.sub(r'<motion[^>]*', '<motionmotionmotionmotionmotionmotiondiv', content)
content = content.replace('<motionmotionmotionmotionmotionmotionmotiondiv', '<div')
Path('src/components/PlaybackQueueBar.jsx').write_text(content, encoding='utf-8')
print('ok')
